...
public class BasicChatInterface extends JPanel implements KeyListener, WindowListener, ActionListener, ExternalListenerInterface {
	...
	private static BasicChatInterface _instance;
	...
	public BasicChatInterface(NLBus nl) {
	...
	public void terminateSession(Long sid) {
		try {
			if (BasicChatInterface.sid!=null) {
				if (!BasicChatInterface.sid.equals(sid)) NLBus.logger.warn("chat interface received a terminated session with id ("+sid+") different from chat session id: "+BasicChatInterface.sid);
				BasicChatInterface.sid=null;
	...
	private void setMenuBar(Set<String> availableCharacters,String currentCharacter,NLBusConfig config) {
		...
		menu.add(new JMenuItem(new AbstractAction("Load information state from file.") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int returnVal = fc.showOpenDialog(BasicChatInterface.this);
	 ...
	 * @throws Exception 
	 */
	private static BasicChatInterface createAndShowGUI() throws Exception {
		//Create and set up the window.
		window = new JFrame(buildTitleString(BasicChatInterface.chatInterfaceSingleSessionID));
		window.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		window.setState(startMinimized?JFrame.ICONIFIED:JFrame.NORMAL);

		BasicChatInterface newContentPane = getInstance();

		float zoom=BasicChatInterface.nlModule.getConfiguration().getZoomFactorChat();
	...
	private static BasicChatInterface getInstance() {
		return _instance;
	}
	@Override
	...
	public static void main(final String[] args) throws Exception {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				try {
					BasicChatInterface ui = createAndShowGUI();
					ui.startupDM();
